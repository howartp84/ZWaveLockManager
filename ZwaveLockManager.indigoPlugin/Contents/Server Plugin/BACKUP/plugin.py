...
class Plugin(indigo.PluginBase):
	...
	def setUserPin(self, pluginAction):
		...
		indigo.zwave.sendRaw(device=indigo.devices[indigoDev],cmdBytes=codeStr,sendMode=1)
...
########################################
	def zwaveCommandReceived(self, cmd):
		...
		if nodeId not in self.lockIDs:
			self.debugLog(u"Node %s is not a lock - ignoring" % (nodeId))
			return
		else:
			self.debugLog(u"Node ID %s found in lockIDs" % (nodeId))

		bytes = byteListStr.split()

		#if ((bytes[5] == "0A") or (bytes[5] == "40")):			#Add node IDs here for debugging
...
########################################
	...
	def triggerEvent(self,eventType,eventNode,userNo):
		...
		self.debugLog(u"triggerEvent called")
		for trigger in self.events[eventType]:
			#dAddress is the Indigo device ID (12345678)
